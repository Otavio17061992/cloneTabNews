...
test("POST /api/v1/projects with malformed header should return 401", async () => {
    const response = await postProject({ Authorization: process.env.ADMIN_API_KEY });
    expect(response.status).toBe(401);
});

test("POST /api/v1/projects with correct key should return 201", async () => {
    const response = await postProject({ Authorization: `Bearer ${process.env.ADMIN_API_KEY}` });
