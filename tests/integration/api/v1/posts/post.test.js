...
test("Post approval workflow and visibility", async () => {
  ...
  const pendingNoAuth = await fetch(
    "http://localhost:3000/api/v1/posts?status=pending",
  );
  expect(pendingNoAuth.status).toBe(401);

  // 4. Lista posts pendentes COM senha correta (deve aparecer lá)
  const pendingAuth = await fetch(
    "http://localhost:3000/api/v1/posts?status=pending",
    {
  ...
  const approveRes = await fetch(
    `http://localhost:3000/api/v1/posts/${postId}/approve`,
    {
      method: "POST",
      headers: adminHeaders,
    },
