...
test("Comment approval workflow", async () => {
  ...
  const getBeforeApproval = await fetch(
    `http://localhost:3000/api/v1/comments?post_id=${postId}`,
  );
  ...
  const approveRes = await fetch(
    `http://localhost:3000/api/v1/comments/${commentId}/approve`,
    {
      method: "POST",
      headers: adminHeaders,
    },
  ...
  const getAfterApproval = await fetch(
    `http://localhost:3000/api/v1/comments?post_id=${postId}`,
  );
...
test("Comment approval security and edge cases", async () => {
  const nonexistentRes = await fetch(
    "http://localhost:3000/api/v1/comments/999999/approve",
    {
      method: "POST",
      headers: adminHeaders,
    },
  ...
  const wrongSecretRes = await fetch(
    `http://localhost:3000/api/v1/comments/${commentId}/approve`,
    {
      method: "POST",
